...
//			console.log(ob);
			...
			if ( ( Math.abs(pX-cX) + Math.abs(pY-cY) ) < ob.sensitivity ) {
				...
				ob.hoverIntent_t = setTimeout( function(){compare(ev, ob, el);} , ob.interval );
			}
		};

		// A private function for delaying the mouseOut function
		var delay = function(ev,ob,el) {
			ob.hoverIntent_t = clearTimeout(ob.hoverIntent_t);
			ob.hoverIntent_s = 0;
		...
		var handleHover = function(e) {
			...
			if (e.type == "mouseenter") {
				...
				$.each(cfgList, function() {
					var ob = this;
					if (ob.hoverIntent_s != 1) { ob.hoverIntent_t = setTimeout( function(){compare(ev,ob,el);} , ob.interval );}
				});
			// else e.type == "mouseleave"
				...
				$.each(cfgList, function() {
					var ob = this;
					if (ob.hoverIntent_s == 1) { ob.hoverIntent_t = setTimeout( function(){delay(ev,ob,el);} , ob.timeout );}
				});
			}
		};
